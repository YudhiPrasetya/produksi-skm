...
?>
        ...
        for($a=$JumlahKolomSizeMinNew; $a<=$JumlahKolomSizeMaxNew; $a++){
         ...
         ?>
         <!-- <div> -->
           <table style="float: left; font-size: 12px" border='1' width='19.7%' cellpadding=2 cellspacing=0 >
                <tr>
                    <th width="4%">SIZE</th>
                    <th colspan="5"><?= $pilih2['size'].$pilih2['cup'] ?></th>
                </tr>
                <tr> 
                    <th>QTY</th>
                    <th colspan=5><?= $pilih2['qty_order'] ?></th>
                </tr>
                <tr>
                    <th width="2%">NO</th>
                    <th width="5%" colspan=2>TTM</th>
                    <th width="5%" colspan=2 style="background: yellow">PKG</th>
                    <th width="7%" style="background: yellow">DATE</th>
                </tr>
            <?php
                    ...
                    $bundle_data = tampilkan_bundle_orc_nomer_new_transaksi_packing_bundle($id_order_detail, $offsetbundle, $limit_baris, $tgl);
                    while($pilih3 = mysqli_fetch_array($bundle_data)){
    
                ?>
               
            <tr height="23px">
                <td <?php if($pilih3['total_input'] == $pilih3['qty_isi_bundle']) { ?> style="background: #7FFFD4" <?php } ?> ><center><a href="print_ticket_pecah_bundle.php?id=<?= $id_order ?>&barcode=<?= $pilih3['barcode_bundle']; ?>&proses=<?= $proses ?>&balance=<?= $pilih3['balance_output']; ?>" target="_blank"> <?= $pilih3['no_bundle']; ?> </a></center></td>
                <td <?php if($pilih3['total_input'] == $pilih3['qty_isi_bundle']) { ?> style="background: #7FFFD4" <?php } ?>><center><?= $pilih3['total_input']; ?></center></td>
                <td <?php if($pilih3['total_input'] == $pilih3['qty_isi_bundle']) { ?> style="background: #7FFFD4" <?php } ?>><center><?= $pilih3['balance_input']; ?></center></td>
                <td <?php if($pilih3['total_output'] == $pilih3['qty_isi_bundle']) { ?> style="background: #7FFFD4" <?php } ?>><center><?= $pilih3['total_output']; ?></center></td>
                <td <?php if($pilih3['total_output'] == $pilih3['qty_isi_bundle']) { ?> style="background: #7FFFD4" <?php } ?>><center><?= $pilih3['balance_output']; ?></center></td>
                <td <?php if($pilih3['total_output'] == $pilih3['qty_isi_bundle']) { ?> style="background: #7FFFD4" <?php } ?>><center><?= tgl_indonesia4($pilih3['tanggal_max']); ?></td>
                <?php
            ...
            <tr>
                ...
                <td style="background: yellow"><center><b><?= $jumlah_bundle_size ?> BOX</b></center></td>
            </tr>
        </table>
        <?php 
        $total_input = 0;
