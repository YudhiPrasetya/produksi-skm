...
<html>
    ...
    <body>
        ...
        <?php 
         ...
         $proses3 = mencari_no_urutan_proses($proses, $id_order);
        $dataproses = mysqli_fetch_array($proses3);
        ...
        for($a=$JumlahKolomSizeMinNew; $a<=$JumlahKolomSizeMaxNew; $a++){
         ...
         <!-- <div> -->
           <table style="float: left; font-size: 12px" border='1' width='19.7%' cellpadding=2 cellspacing=0 >
                ...
                <tr>
                    <th width="2%">NO</th>
                    <th width="5%" colspan=2>BFR</th>
                    <th width="5%" colspan=2 style="background: yellow">TTM</th>
            ...
            <?php
                    ...
                    $bundle_data = tampilkan_bundle_orc_nomer_new_transaksi_tatami($id_order_detail, $offsetbundle, $limit_baris, $tgl, $table_transaksi_sebelum);
                    while($pilih3 = mysqli_fetch_array($bundle_data)){
    
                ?>
               
            <tr height="23px">
            <td <?php if($pilih3['total_input'] == $pilih3['qty_isi_bundle']) { ?> style="background: #7FFFD4" <?php }elseif(($pilih3['total_input'] > 0) && ($pilih3['total_input'] < $pilih3['qty_isi_bundle'])){ ?> style="background: #FFB6C1" <?php } ?> ><center><php?id=<?= $id_order ?>&barcode=<?= $pilih3['barcode_bundle']; ?>&proses=<?= $proses ?>&balance=<?= $pilih3['balance_output']; ?>" target="_blank"> <?= $pilih3['no_bundle']; ?> </a></center></td>
