     ...
     <?php 
         ...
         <td width="180px">
          ...
          <label class="form-check-label" style="font-weight: normal" for="inlineCheckbox<?= $i; ?>"><?= strtoupper(
            ($data2['nama_transaksi'] == "washing" ? "f qc" : ($data2['nama_transaksi'] == "qc_buyer" ? "furushima" :($data2['nama_transaksi'] == "press" ? "padprint" : ($data2['nama_transaksi'] == "bemis" ? "fuse" : $data2['nama_transaksi']))))
            ) ?></label>
        </td>
        <?php }else{ ?>
      </tr>
