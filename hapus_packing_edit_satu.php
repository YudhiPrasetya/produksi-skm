...
if(isset($_GET['id'])){
  ...
  // $id = max($dataid);
    ...
    if($qty_temp <= 1){
      ...
      if(update_kurangi_qty_transaksi_packing($tanggal, $jam, $id, $update_qty_delete)) {
        $_SESSION['pesan']='Data Packing Berkurang Satu';
        header("Location: edit_packing.php?trx=".$trx);
      }else{
      echo "gagal menghapus data";
