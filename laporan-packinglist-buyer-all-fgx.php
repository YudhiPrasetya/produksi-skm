...
<?php
   ...
   $ctk =array();
    ...
    while($pilih = mysqli_fetch_array($laporan)){
      $ctk[$pilih['no_po']][$pilih['style']][$pilih['warna']][$pilih['description']][$pilih['no_contract']][$pilih['costomer']][$pilih['kelompok_size']][$pilih['ukuran_karton']][]=array(
      'karton'=>$pilih['karton'],
...
<?php
  ...
  foreach($color as $warna=>$desc)
  foreach($desc as $description=>$contract)
  foreach($contract as $no_contract=>$buyer)
  foreach($buyer as $costomer=>$kelompok)
  foreach($kelompok as $kelompoksize=>$ukuranKarton)
...
<table class='hlap' width='100%'>
  ...
  <tr>
      <td width='12%'>Contract Number</td><td width='1%'>:</td><td width='15%' align='left'><?= $no_contract ?> </td>
      <td width='45%'></td>
