...
<table  border='1' class='table table-striped table-hover' width=100% cellpadding=6 >
  ...
  <tbody>
    <?php
      $no=1;
      $laporan2 = tampilkan_laporan_packinglist_perstyle_invoice_mix($po, $style, $invoice);
        while($pilih2 = mysqli_fetch_array($laporan2)){
      $no_to = $no+$pilih2['karton']-1;
    ?>
    <tr class="belang">
      <td align='center'><?= $no ; ?> - <?= $no_to ?></td>
      ...
      <td align='center'><?= $pilih2['gross']; ?></td>
      <td align='center'><?= $pilih2['nett']; ?></td>
      <td align='center'><?= $pilih2['karton']*0.09 ?></td>
    </tr>
      <?php 
      ...
      ?>
    <tr class="belang">
      <td style='background-color:#20B2AA; color: #ffffff; font-weight:bold' colspan='2' >Total QTY :</td>
...
<table width="100%" style="background-color:yellow; font-weight:bold">
    ...
    <tr>
        ...
        <td align="right">    
            NETT WEIGHT : <?= number_format($total_gross_semua,2) ?> KG
        </td>
    </tr>
</tr>
</table>
