 ...
 <h3>LAPORAN PACKING LIST</h3>
  <h4>
    <?php
        ...
        $kubik = explode (" x ",$data4['ukuran_karton']);
        $kubik_int = array_map('intval', $kubik);
        $jmlh_array = count($kubik_int);
        $hasil_kali = 1;
        for($i=0; $i<$jmlh_array; $i++){
          $hasil_kali = $hasil_kali * $kubik_int[$i];
        }
        $volume_karton = $hasil_kali;
...
<b><u>TOTAL SHIPMENT SEMUANYA : </u></b> <br> </center>
  ...
  <br>
  TOTAL CBM : <?php 
    $cbm = $volume_karton/1000000*$total_semua;
    echo $cbm;
  ?>
