...
?>
    ...
    <h4>Nomer Purchasing Order : <?= $data['no_po']; ?></h4>
    </center>
...
$no_karton3=0;
$ctk =array();
  $laporan = tampilkan_laporan_shipment_packinglist_po_invoice($po, $invoice);
  while($pilih = mysqli_fetch_array($laporan)){
    $ctk[$pilih['no_po']][$pilih['style']][$pilih['description']][$pilih['warna']][$pilih['kelompok_size']][]=array(
      'karton'=>$pilih['karton'],
...
<tr>
  <td>
<?php
  foreach($ctk as $no_po=>$style2)
  foreach($style2 as $style=>$desc)
  foreach($desc as $description=>$warna)
  foreach($warna as $color=>$kelompok)
...
<table class='hlap' width='100%'>
  ...
  <tr>
    <td width='12%'>Style</td><td width='1%'>:</td><td width='15%' align='left'><?= $style.' ( '. $color ?> )</td>
    <td width='45%'></td>
...
<br>
<?php
  $laporan2 = tampilkan_laporan_shipment_po_hidesize($invoice, $po);
