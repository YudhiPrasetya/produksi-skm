...
$username = $_SESSION['username'];
  ...
  if(!empty(trim($style)) && !empty(trim($qty_karton))){
    if(cek_style_karton($style) == 0){
    if(tambah_data_master_qty_karton($style, $qty_karton, $username)){
      $error = "Data Berhasil di simpan";
    }else {
      $error = "Gagal Menyimpan data";
    }
  }else{
    $error = "QTY Karton di Style ini udah dimasukkan";
  }
  }else{
...
<div class="row" style="margin-left: 20px; margin-right: 20px">
 ...
 <div class="col-sm-4 tetep"> 
     <br><br>
 <form method="post">
   <h3><center>Tambah Data Master Full Karton</center></h3>
   <br><br><br>
   ...
   <div class="form-group">
   <label for="qty_karton">QTY KARTON</label>
   <div class="input-group">
     <div class="input-group-addon">
     <i class="glyphicon glyphicon-list-alt"></i>
   </div>
      <input type="number" class="form-control" placeholder="Masukkan Qty Karton" name="qty_karton" id="qty_karton" required>
   </div>
...
<table border="1px" class="table table-striped table-bordered data">
  ...
  <tr>
    ...
    <th class="tengah theader" style="text-align: center">QTY KARTON</th>
...
// $subtotal_qty += $row['qty'];
  ...
  <td class="tengah"><?= $row['qty_karton']; ?></td>
