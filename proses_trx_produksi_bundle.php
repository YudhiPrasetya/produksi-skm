  ...
  require_once 'core/init.php';
    ...
    if(cek_ketersediaan_proses($id_order, $proses) != 0){
        ...
        if($proses === 'packing_bundle'){
            ...
            $datatemp = cek_jumlah_qty_scan_temp_trx($no_trx);
            $datascan = mysqli_fetch_array($datatemp);
            $total_scan_trx = $datascan['total_scan'];
            $balance_ctn = $qty_karton - $total_scan_trx;
            if($qty_scan > $balance_ctn){
                $qty_scan = $balance_ctn;
            }
        ...
        if(($qty_scan <= $qty_before) && ($qty_before != 0) ){
