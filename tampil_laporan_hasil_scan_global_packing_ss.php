...
$sql = "SELECT C.tanggal_max, A.costomer, A.id_order, A.no_po, A.orc, A.style, A.color, A.qty_order,
ifnull(D.output_total,0) qty_tatami, ( ifnull(D.output_total,0) - ifnull(C.output_total,0)) wip_tatami, ifnull(B.daily,0) daily,
...
sum(A.qty_isi_bundle) qty_order, C.status, F.category, G.plan_line FROM master_bundle A
 ...
 LEFT OUTER JOIN 
 (SELECT A.tanggal, C.id_order, sum(ifnull(A.qty,0)) daily FROM transaksi_packing_bundle A
 ...
 ON A.id_order = B.id_order
  LEFT OUTER JOIN 
 (SELECT max(A.tanggal) tanggal_max, C.id_order, sum(ifnull(A.qty,0)) output_total FROM transaksi_packing_bundle A
 JOIN master_bundle B ON A.kode_barcode = B.barcode_bundle
 JOIN order_detail C ON B.id_order_detail = C.id_order_detail
 WHERE tanggal <= '$tgl' 
 ...
 ON A.id_order = AD.id_order
  ";
if($checkstyle == 'iya'){
  $sql .= " WHERE A.orc LIKE '%$orc%' AND A.style = '$style' AND AC.table_transaksi = 'transaksi_packing_bundle' AND A.status = '$status' 
  AND A.costomer like '%$costomer%' AND A.category LIKE '%$category%' AND IFNULL(AD.line, 'not_yet') LIKE '%$line%'
  AND A.no_po LIKE '%$no_po%'";
 }else{
  $sql .= " WHERE A.orc LIKE '%$orc%' AND A.style LIKE '%$style%' AND AC.table_transaksi = 'transaksi_packing_bundle' AND A.status = '$status' 
  AND A.costomer like '%$costomer%' AND A.category LIKE '%$category%' AND IFNULL(AD.line, 'not_yet') LIKE '%$line%'
  AND A.no_po LIKE '%$no_po%' ";
 }

$sql1 = "SELECT A.id_order FROM 
( SELECT C.id_order,  C.status, E.costomer, C.orc, D.style, C.no_po
...
if($checkstyle == 'iya'){
  $where =" WHERE A.status = '$status' AND A.costomer like '%$costomer%' AND A.orc LIKE '%$orc%' AND A.style = '$style' AND AC.table_transaksi = 'transaksi_packing_bundle'
  AND IFNULL(AD.line, 'not_yet') LIKE '%$line%' AND A.no_po LIKE '%$no_po%'";
 }else{
   $where =" WHERE A.status = '$status' AND A.costomer like '%$costomer%' AND A.orc LIKE '%$orc%' AND A.style LIKE '%$style%' AND AC.table_transaksi = 'transaksi_packing_bundle'
   AND IFNULL(AD.line, 'not_yet') LIKE '%$line%' AND A.no_po LIKE '%$no_po%'";
 }
...
{
$sql .= "  ORDER BY D.tanggal_max desc, B.daily asc";
}
...
$nestedData['qty_tatami'] = $r['qty_tatami'];
$nestedData['wip_tatami'] = $r['wip_tatami'];
$nestedData['daily'] = $r['daily'];
$nestedData['total'] = $r['total'];
$nestedData['balance'] = $r['balance'];
$nestedData['aksi'] = '<button  id="edit" data-toggle="modal" data-target="#myEdit" style="width: 30px; padding: 0; margin: 0" class="edit_material btn btn-primary edit_komentar kecil" data-id="'.$r['id_order'].'" data-table="transaksi_packing_bundle"><i class="glyphicon glyphicon-zoom-in"></i></button> | 
<a href="bundle_record.php?id='.$r['id_order'].'&tgl='.$tgl.'&proses=packing_bundle" target="_blank"><i class="glyphicon glyphicon-open"></i></a> |
<a href="bundle_record_packing.php?id='.$r['id_order'].'&tgl='.$tgl.'&proses=packing_bundle"; target="_blank"><i class="glyphicon glyphicon-th-large"></i></a>
';
