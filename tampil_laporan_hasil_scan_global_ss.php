...
sum(A.qty_isi_bundle) qty_order, C.status, F.category, G.plan_line FROM master_bundle A
 ...
 WHERE C.orc LIKE '%$orc%' AND D.style LIKE '%$style%' AND C.status = '$status' AND E.costomer like '%$costomer%'
 AND F.category LIKE '%$category%' AND C.no_po LIKE '%$no_po%'
 GROUP BY C.id_order
...
<a href="bundle_record.php?id='.$r['id_order'].'&tgl='.$tgl.'&proses='.$proses.'" target="_blank"><i class="glyphicon glyphicon-open"></i></a> 
';
if($proses == 'qc_endline'){ 
    $nestedData['aksi'] .= '| <a href="bundle_record_sewing.php?id='.$r['id_order'].'&tgl='.$tgl.'&proses='.$proses.'"; target="_blank"><i class="glyphicon glyphicon-th-large"></i></a>';
}
if($proses == 'qc_buyer'){ 
    $nestedData['aksi'] .= '| <a href="bundle_record_qc_buyer.php?id='.$r['id_order'].'&tgl='.$tgl.'&proses='.$proses.'"; target="_blank"><i class="glyphicon glyphicon-th-large"></i></a>';
}
if($proses == 'tatami'){ 
  $nestedData['aksi'] .= '| <a href="bundle_record_tatami.php?id='.$r['id_order'].'&tgl='.$tgl.'&proses='.$proses.'"; target="_blank"><i class="glyphicon glyphicon-th-large"></i></a>';
}
