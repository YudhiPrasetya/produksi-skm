...
?>    
    <td>
    <form action="tampil_laporan_stok_packing_barcode_buyer2.php" method="post" target="_blank">
...
<?php if($costomer != '') { 
    if(cek_jumlah_size_transaksi_packing_mixstyle($tgl, $costomer) !=0) {
     $ListSizeMix = tampilkan_size_transaksi_packing_mixstyle2($tgl, $costomer);
...
<form action="tampil_laporan_stok_packing_barcode_buyer_mixstyle.php" method="post" target="_blank">
<input type="hidden" value="<?= $tgl ?>" name="tgl" id="tgl3">
<input type="hidden" value="<?= $costomer ?>" name="costomer"  id="costomer3">
