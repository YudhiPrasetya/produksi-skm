...
<?php 
  $data1 = tampilkan_data_temp_qc_kensa($user);
    ...
    if($temp['data_no'] > 0){
      // $data3 = tampilkan_no_transaksi_qc_kensa0($user);
...
Total Qty Scan :
<?php
    $temp_temp_qc_kensa = tampilkan_temp_qc_kensa($user);
    $subtotal_qty=0;
...
<?php
$temp_temp_qc_kensa = tampilkan_temp_qc_kensa($_SESSION['username']);
?>
<!-- <div class="container"> -->
...
$no=1;
$subtotal_qty=0;
while($row=mysqli_fetch_assoc($temp_temp_qc_kensa))
{ 
...
if (tanya3 == true) return true;
  ...
  <td class="tengah"><a href="hapus_qc_kensa_satu.php?id=<?= $row['id_transaksi_qc_kensa']; ?>" onclick="return konfirmasi_kurangi()">DEL</a></td>
  </tr>
