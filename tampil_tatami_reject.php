...
Total Qty Scan :
<?php
    $temp_reject_total = tampilkan_temp_tatami_reject($user);
    $subtotal_qty=0;
    while($data=mysqli_fetch_assoc($temp_reject_total)){
...
<?php
$temp_reject = tampilkan_temp_tatami_reject($_SESSION['username']);
?>
<!-- <div class="container"> -->
...
$no=1;
$subtotal_qty=0;
while($row=mysqli_fetch_assoc($temp_reject))
{ 
...
if (tanya3 == true) return true;
  ...
  <td class="tengah"><a href="hapus_reject_satu.php?id=<?= $row['ids_to_delete']; ?>" onclick="return konfirmasi_kurangi()">DEL</a></td>
  </tr>
