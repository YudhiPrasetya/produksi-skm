        ...
        <table border="1px" id="example" class="table table-striped table-hover table-bordered display" style="font-size: 13px; width: 100%">
          <thead>
            <tr>
              ...
              <th style="background: #0000FF; text-align: center">NILAI SMV</th>
              ...
              <th style="background: #0000FF; text-align: center">LINE</th>
          ...
          <tbody>
            <?php
              $no=1;
              $shipment = tampilkan_input_target_harian($style, $no_po, $costomer, $today);
              while($row=mysqli_fetch_assoc($shipment))
              {
            ?>
            <tr class="pilih" data-order="<?= $row['orc']; ?>" data-id="<?= $row['id_order']; ?>" data-po="<?= $row['no_po']; ?>" data-smv="<?= $row['nilai_smv']; ?>" data-style="<?= $row['style']; ?>" data-color="<?= $row['color']; ?>" data-jmloperator="<?= $row['jml_operator']; ?>" data-costomer="<?= $row['costomer']; ?>" data-line="<?= $row['plan_line']; ?>" data-dismiss="modal">
              ...
              <td class="tengah"><?= $row['plan_line']; ?></td>
               <!-- <td class="tengah"> -->
              <!-- <?php
...
<script type="text/javascript">
  $(document).on('click', '.pilih', function (e) {
    ...
    document.getElementById("nilai_smv").value = $(this).attr('data-smv');
    document.getElementById("nilai_smv2").value = $(this).attr('data-smv');
    document.getElementById("man_power").value = $(this).attr('data-jmloperator');
