...
$sql = "SELECT C.orc, C.no_po, C.label, D.style, B.warna, A.no_trx, A.tanggal_scan, A.jam_scan, 
...
if($_POST['checkstyle'] == 'iya'){
    $sql .= " WHERE C.no_po LIKE '%$no_po%' AND A.orc LIKE '%$orc%' AND D.style = '$style' AND A.id_shipment = $packinglist";
}else{
    $sql .= " WHERE C.no_po LIKE '%$no_po%' AND A.orc LIKE '%$orc%' AND D.style LIKE '%$style%' AND A.id_shipment = $packinglist";
}

$sql1 = "SELECT C.orc, C.no_po, C.label, D.style, B.warna, A.no_trx, A.tanggal_scan, A.jam_scan, 
group_concat(A.id_transaksi_shipment) AS ids_to_delete,  
A.no_trx
  FROM transaksi_shipment A
  join barang B ON A.kode_barcode = B.kode_barcode
...
if($_POST['checkstyle'] == 'iya'){
    $where =  " WHERE C.no_po LIKE '%$no_po%' AND A.orc LIKE '%$orc%' AND D.style = '$style' AND A.id_shipment = $packinglist 
    group by A.no_trx, A.orc, D.style, B.warna";
}else{
    $where =  " WHERE C.no_po LIKE '%$no_po%' AND A.orc LIKE '%$orc%' AND D.style LIKE '%$style%' AND A.id_shipment = $packinglist 
    group by A.no_trx, A.orc, D.style, B.warna";
}
...
$sql .= " AND ( A.no_trx LIKE '%".$search_value."%' OR A.tanggal_scan LIKE '%".$search_value."%'
...
{
$nestedData['id_trx'] = '<input type="checkbox" class="check-item" name="idtrx" value="'.$r['no_trx'].'">';
$nestedData['no_trx'] = $r['no_trx'];
$nestedData['tanggal'] = $r['tanggal_scan'];
